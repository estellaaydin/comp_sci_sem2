...
class starter {
	...
	public static void searchArrInt(ArrayList<Integer> arr, int num){
		for(int x = 0; x<arr.size();x++){
			if(arr.get(x) == num){
				System.out.println("the number " + num + " is at index " + x);
			}
			else{
				System.out.println("there is no number " + num );
			}
		}
	}
	...
	public static void searchArrString(ArrayList<String> arr, String a){
		for(int y =0; y<arr.size(); y++){
			if(arr.get(y).equals (a)){
				System.out.println("the word " + a + " is at index "+ y);
			}
			else{
				System.out.println("there is no word " + a);
			}
		}
	}
}
